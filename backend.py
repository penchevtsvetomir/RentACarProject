...
def add_car():
    ...
    # car_id = int(input("Enter a car ID: ")) # Car ID да се задава автоматично, иначе може да се дубликира с вече съществуващо
    ...
    new_car = {
        "id": CARS[-1]["id"] + 1, # Car ID да се задава автоматично, иначе може да се дубликира с вече съществуващо
        "brand": car_brand,
        "model": car_model,
...
def return_car(car_id):
    ...
    for car in CARS:
        if car_id == car["id"] and car["available"] is False:
            car["available"] = True
            return "You successfully returned the car!"
    return "No such car found or car is not rented! Please try again."


def calculate_total(car_id, mileage):
